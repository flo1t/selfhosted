...
#### Limit log file size
`nano ~/docker/crodwsec/config/config.yaml`
```yaml
...
common:
...
#### Add Traefik bouncer
Generate an API-Key:
`docker exec -t crowdsec cscli bouncers add bouncer-traefik-<hostname>`
...
`docker run crowdsecurity/cloudflare-worker-bouncer -g <API-Token> > ~/docker/crowdsec/cloudflare-bouncer/cloudflare-worker-bouncer.yaml`

Check if the LAPI_URL is correct and add the Crowdsec Bouncer API-Key from the first step to the config:
`nano ~/docker/crowdsec/cloudflare-bouncer/cloudflare-worker-bouncer.yaml`
```
...
Change worker route fail mode within Cloudflare -> Wrokers Routes (https://docs.crowdsec.net/u/bouncers/cloudflare-workers)

Prioritize local decision-making (only necessary with the CloudFlare free plan)
`nano docker/crowdsec/cloudflare-bouncer/cloudflare-worker-bouncer.yaml`
```
...
...
Add the configuration from setup-files/http.yml to `sudo nano ~/docker/crowdsec/config/notifications/http.yaml` to enable notifications via Discord (replace <Discord URL> with the webhook URL).
