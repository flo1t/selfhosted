...
### Monitoring
I'm monitoring Jellyfin with Grafana, Vector and VictoriaMetrics (flux). The configuration files will be added to rpi01\monitoring\ in a later stage.

### Limit rate per IP
#### Enable log rotate
`sudo nano /etc/logrotate.d/jellyfin-nginx`
```sh
/home/floit/docker/jellyfin/nginx/logs/*.log
{
        su root root
        daily
        maxsize 20M
        ...
        dateformat -%Y%m%d%H
        postrotate
            docker kill --signal="USR1" jellyfin-proxy
        endscript
}
```

Test it: `sudo logrotate -f /etc/logrotate.d/jellyfin-nginx --debug`
Optional: Execute it once: `sudo logrotate -f /etc/logrotate.d/jellyfin-nginx`

### ToDo
- Implement Authentik for Jellyserr
- Add monitoring configuration files to rpi01/monitoring
